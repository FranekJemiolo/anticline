...
import { OPFSStorageEngine, DatasetManifest, PyodideETLRunner } from '@franekjemiolo/orogen';
...
async function bootstrap() {
  ...
  new DatasetRegistry(registryPanel, {
    onSubscribeManifest: async (manifest: DatasetManifest) => {
      console.log(`Subscribed to P2P swarm for dataset: ${manifest.name}`);
    },
    onExecuteETL: async (manifest: DatasetManifest) => {
  ...
  const crdtRoom = new CRDTRoom({
    ...
    onPresenceChanged: (peers) => {
      ...
      if (pEl) pEl.textContent = `Active Analysts: ${peers.length}`;
    },
  });
