export class TurnSettingsModal {
    ...
    `;
        ...
        modalEl.querySelector('#btn-save-turn')?.addEventListener('click', () => {
            const urls = (modalEl.querySelector('#turn-urls').value || '')
                .split('\n')
                .map((u) => u.trim())
                .filter(Boolean);
            const username = modalEl.querySelector('#turn-user').value.trim();
            const credential = modalEl.querySelector('#turn-pass').value.trim();
            const config = {
                urls,
                username,
                credential,
