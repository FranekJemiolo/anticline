export class VisualJoinBuilder {
    ...
    bridge;
    onJoinExecuted;
    primaryTable = 'intraday_trades';
    macroTable = 'macro_us_treasury';
    toleranceSeconds = 86400; // 24 hours tolerance for daily macro join
...
ORDER BY t.timestamp ASC;`;
    ...
    attachEvents() {
        ...
        if (macroCard && primaryCard) {
            macroCard.addEventListener('dragstart', (e) => {
                e.dataTransfer.setData('text/plain', this.macroTable);
            });
            primaryCard.addEventListener('dragover', (e) => {
                e.preventDefault();
            ...
            primaryCard.addEventListener('drop', (e) => {
                e.preventDefault();
                primaryCard.classList.remove('drag-hover');
                this.executeJoin();
            });
