...
import { CandlestickPoint, TimeSeriesPoint } from '../../types/index.js';

export class ChartContainer {
  private container: HTMLElement;
  private chart: IChartApi | null = null;
  private candleSeries: ISeriesApi<'Candlestick'> | null = null;
  private macroSeries: ISeriesApi<'Line'> | null = null;
   ...
   */
  ingestArrowTable(table: Table): void {
    ...
    if (openCol && highCol && lowCol && closeCol && this.candleSeries) {
      ...
      for (let i = 0; i < table.numRows; i++) {
        ...
        if (t > 1e11) t = Math.floor(t / 1000); // normalize ms to seconds
        candles.push({
          time: t,
    ...
    if (macroCol && this.macroSeries) {
      const macroPoints: TimeSeriesPoint[] = [];
      for (let i = 0; i < table.numRows; i++) {
        let t = Number(timeCol.get(i));
      ...
      this.macroSeries.setData(macroPoints as any);
    }
  }
  ...
  updateLatestCandle(candle: CandlestickPoint): void {
    if (this.candleSeries) {
      this.candleSeries.update(candle as any);
    }
  }

  setMacroSeries(points: TimeSeriesPoint[]): void {
    if (this.macroSeries) {
      this.macroSeries.setData(points as any);
    }
  }
   ...
   */
  renderAnnotations(annotations: Array<{ x1: number; y1: number; x2: number; y2: number; color: string; label?: string }>): void {
