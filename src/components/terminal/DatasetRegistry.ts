import { DatasetManifest } from '@franekjemiolo/orogen';

export interface DatasetRegistryOptions {
  onSubscribeManifest: (manifest: DatasetManifest) => void;
  onExecuteETL: (manifest: DatasetManifest) => void;
  corsProxyUrl?: string;
  fredApiKey?: string;
}
...
export class DatasetRegistry {
  ...
  private options: DatasetRegistryOptions;
  private activeTab: 'network' | 'warehouse' = 'network';
  private selectedAssetClass: string = 'all';
  private selectedEngine: string = 'all';
  private subscribedDatasetIds: Set<string> = new Set([
    'fred-t10y2y-spread',
    'binance-btc-1m',
  ]);

  private availableDatasets: DatasetManifest[] = [
    {
      ...
      etl: {
        engine: 'duckdb-sql',
        source_api:
          'https://api.stlouisfed.org/fred/series/observations?series_id=T10Y2Y&file_type=json',
        script:
          "WITH raw_data AS (SELECT UNNEST(observations) AS obs FROM read_json_auto('{{CORS_PROXY}}https://api.stlouisfed.org/fred/series/observations?series_id=T10Y2Y&file_type=json&api_key={{FRED_API_KEY}}')) SELECT CAST(obs.date AS TIMESTAMP) AS date, CAST(NULLIF(obs.value, '.') AS FLOAT32) AS spread FROM raw_data WHERE obs.value != '.';",
      },
    },
    {
      ...
      description:
        '255 days of daily EUR/PLN midpoint exchange rates from National Bank of Poland',
      asset_class: 'macro',
      schema: [
        { column: 'date', type: 'TIMESTAMP' },
      ...
      etl: {
        engine: 'duckdb-sql',
        source_api:
          'http://api.nbp.pl/api/exchangerates/rates/a/eur/last/255/?format=json',
        script:
          "WITH raw_data AS (SELECT UNNEST(rates) AS rate_record FROM read_json_auto('http://api.nbp.pl/api/exchangerates/rates/a/eur/last/255/?format=json')) SELECT CAST(rate_record.effectiveDate AS TIMESTAMP) AS date, CAST(rate_record.mid AS FLOAT32) AS rate FROM raw_data;",
      },
    },
    {
      ...
      description:
        '10 years of daily OHLCV bar data from Yahoo Finance via Pyodide Arrow IPC',
      ...
      etl: {
        ...
        script:
          '# Yahoo Finance SPY 10-Year Daily Pyodide Pipeline\nimport yfinance as yf\nresult_arrow = None',
      },
    },
    {
      id: 'binance-btc-1m',
      ...
      description:
        'High-volume 1-minute candlestick data paginated with 0.5s rate-limit backoffs',
      asset_class: 'crypto',
      schema: [
      ...
      etl: {
        ...
        script:
          '# Binance 1000-candle paginated batches with rate limiting\nresult_arrow = None',
      },
    },
    {
      id: 'sec-edgar-aapl',
      ...
      description:
        'Standardized quarterly NetIncomeLoss and Revenues directly from US SEC EDGAR XBRL company facts API',
      ...
      etl: {
        ...
        script:
          '# SEC EDGAR XBRL Pipeline with User-Agent Injection\n# Extracts NetIncomeLoss & Revenues into Apache Arrow\nresult_arrow = None',
      },
    },
    {
      ...
      description:
        'Localized discretionary inflation proxy scraping Warsaw garden furniture prices with European format cleaning',
      ...
      etl: {
        engine: 'pyodide-python',
        source_api:
          'https://www.example-polish-retailer.pl/kategoria/meble-ogrodowe?lokalizacja=warszawa',
        requirements: ['beautifulsoup4', 'requests', 'pandas', 'pyarrow'],
        script:
          '# BeautifulSoup4 scraping pipeline through CORS proxy\n# Cleans Polish currency non-breaking spaces and comma decimals\nresult_arrow = None',
      },
    },
    {
      ...
      description:
        'Daily Wikimedia pageviews for "Recession" over 5 years mapping retail curiosity to market cycles',
      ...
      etl: {
        engine: 'duckdb-sql',
        source_api:
          'https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/en.wikipedia/all-access/all-agents/Recession/daily/20190101/20240101',
        script:
    ...
    },
    {
      ...
      description:
        'Historical daily Total Value Locked (TVL) in USD across all blockchains tracking macro DeFi liquidity',
      asset_class: 'crypto',
      schema: [
        { column: 'date', type: 'TIMESTAMP' },
      ...
      etl: {
        ...
        script:
          'SELECT CAST(to_timestamp(CAST(date AS BIGINT)) AS TIMESTAMP) AS date, CAST(totalLiquidityUSD AS FLOAT64) AS totalLiquidityUSD FROM read_json_auto(\'https://api.llama.fi/charts\');',
      },
    },
    {
      id: 'ecb-eur-usd',
      ...
      description:
        'Daily official EUR/USD foreign exchange reference rates from European Central Bank Data Portal API get_observations endpoint',
      asset_class: 'fx',
      schema: [
      ...
      etl: {
        engine: 'duckdb-sql',
        source_api:
          'https://data-api.ecb.europa.eu/service/data/EXR/D.USD.EUR.SP00.A/get_observations?format=jsondata',
        script:
          "SELECT CAST(period AS TIMESTAMP) AS period, CAST(value AS FLOAT32) AS value FROM read_json_auto('https://data-api.ecb.europa.eu/service/data/EXR/D.USD.EUR.SP00.A/get_observations?format=jsondata');",
      },
    },
    {
      id: 'fred-ice-bofa-hy',
      ...
      description:
        'Option-Adjusted Spread of ICE BofA US High Yield Index (BAMLH0A0HYM2) tracking corporate credit risk over Treasuries',
      ...
      etl: {
        engine: 'duckdb-sql',
        source_api:
          'https://api.stlouisfed.org/fred/series/observations?series_id=BAMLH0A0HYM2&file_type=json',
        script:
    ...
    },
    {
      ...
      description:
        'Annual Gross Domestic Product in current USD (NY.GDP.MKTP.CD) for United States and European Union',
      asset_class: 'macro',
      schema: [
        { column: 'date', type: 'TIMESTAMP' },
      ...
      etl: {
        engine: 'duckdb-sql',
        source_api:
          'http://api.worldbank.org/v2/country/US;EU/indicator/NY.GDP.MKTP.CD?format=json&per_page=1000',
        script:
    ...
    },
    {
      ...
      description:
        'Monthly global commodity price benchmark indices covering energy, industrial metals, and agriculture',
      ...
      etl: {
        engine: 'pyodide-python',
        source_api:
          'https://raw.githubusercontent.com/datasets/commodity-prices/master/data/commodity-prices.csv',
        requirements: ['requests', 'pandas', 'pyarrow'],
        script:
          '# World Bank Pink Sheet Monthly Commodities Pipeline\nresult_arrow = None',
      },
    },
    {
      ...
      description:
        '10 years of daily precipitation sum data for Minas Gerais, Brazil (primary coffee production basin)',
      asset_class: 'alternative',
      schema: [
      ...
      etl: {
        engine: 'duckdb-sql',
        source_api:
          'https://archive-api.open-meteo.com/v1/archive?latitude=-18.5122&longitude=-44.5550&start_date=2014-01-01&end_date=2024-01-01&daily=precipitation_sum&timezone=America%2FSao_Paulo',
        script:
  ...
  private attachEvents(): void {
    ...
    this.container.querySelectorAll('.tab-btn').forEach((btn) => {
      btn.addEventListener('click', () => {
        const tab = (btn as HTMLElement).dataset.tab as 'network' | 'warehouse';
        if (tab) {
          this.activeTab = tab;
          this.render();
        }
    ...
    this.container.querySelectorAll('.filter-chip').forEach((chip) => {
      chip.addEventListener('click', () => {
        const filter = (chip as HTMLElement).dataset.filterClass;
        if (filter) {
    ...
    this.container.querySelectorAll('.btn-sub').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        ...
        if (ds) {
          if (
            ds.etl.script.includes('{{FRED_API_KEY}}') &&
  ...
  private openLineageModal(manifest: DatasetManifest): void {
  ...
  private promptApiKey(provider: string, onConfirmed: () => void): void {
    ...
    backdrop.querySelector('#save-apikey-btn')?.addEventListener('click', () => {
      const input = backdrop.querySelector('#vault-key-input') as HTMLInputElement;
   ...
   */
  private resolveRuntimeVariables(manifest: DatasetManifest): DatasetManifest {
   ...
   */
  private async pollRemoteRegistry(): Promise<void> {
    const remoteUrl =
    ...
    try {
      ...
      if (resp.ok) {
        ...
        if (data && data.datasets && data.datasets.length > 0) {
          console.log(
            `Discovered ${data.datasets.length} remote dataset manifests from ${remoteUrl}`
          );
        }
      }
    } catch {
